...
describe('extractReferences', () => {
  ...
  it('не захватывает хвостовую пунктуацию в номер акта', () => {
    // «149-ФЗ...» в конце предложения не должен давать номер «149-ФЗ...»:
    // такой номер не совпадёт ни с одной записью корпуса.
    const withEllipsis = firstOf('статьи 15 Федерального закона от 27.07.2006 № 149-ФЗ...');
    expect(withEllipsis.number).toBe('149-ФЗ');

    const withPeriod = firstOf('согласно Федеральному закону от 27.07.2006 № 149-ФЗ.');
    expect(withPeriod.number).toBe('149-ФЗ');

    const withComma = firstOf('в Федеральном законе от 27.07.2006 № 149-ФЗ, а также');
    expect(withComma.number).toBe('149-ФЗ');
  });

  it('сохраняет составные номера актов', () => {
    const ref = firstOf('распоряжение Правительства Российской Федерации от 17.01.2026 № 28-р');
    expect(ref.number).toBe('28-р');
  });

  it('не считает ссылкой заголовок структурной единицы', () => {
    // «Статья 1. Предмет регулирования» объявляет структуру, а не ссылается.
    // Без этой проверки разбор закона выдаёт ложную ссылку на каждую статью.
    const refs = extractReferences('Статья 1. Предмет регулирования\n\nТекст нормы.');
    expect(refs).toEqual([]);

    const chapter = extractReferences('Глава 2. Заключительные положения\n\nТекст.');
    expect(chapter).toEqual([]);
  });

  it('различает заголовок и ссылку на ту же статью', () => {
    const text = 'Статья 5. Порядок\n\nПорядок установлен статьёй 5 настоящего закона.';
    const refs = extractReferences(text, {
      selfAct: { type: 'federal-law', number: '1-ФЗ', date: '2020-01-01' },
    });
    // Остаётся ровно одна ссылка — из текста, а не из заголовка.
    expect(refs).toHaveLength(1);
    expect(refs[0]!.span[0]).toBeGreaterThan(text.indexOf('Порядок установлен'));
  });

  it('отбрасывает пустую ссылку на сам акт, когда акт неизвестен', () => {
    // «настоящий Федеральный закон» без структурной единицы и без selfAct
    // ни к чему не адресует и в результате не нужен.
    expect(extractReferences('в соответствии с настоящим Федеральным законом')).toEqual([]);
  });
