 ...
 *
 * ⚠️ ТОЛЬКО ДЛЯ РАЗРАБОТКИ И ИССЛЕДОВАНИЯ. Это сторонний зарубежный сервис:
 * всё, что через него загружается, становится ему видно, а исходящее
 * соединение к зарубежному узлу в аттестованном контуре недопустимо.
 * В продуктиве используйте прямой доступ или прокси, размещённый в России
 * (см. `HttpFetcher` с параметром `proxyUrl`).
 *
